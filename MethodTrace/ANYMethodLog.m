...
#define TARGET_LOG(format, ...) TLog(format, ## __VA_ARGS__)
#else
...
#ifdef OPEN_TLog
#define TLog(format, ...) TLog(format, ## __VA_ARGS__)
#else
#define TLog(format, ...)
#endif
...
BOOL qhd_isCanHook(Method method, const char *returnType) {
    ...
    return isCanHook;
        ...
        TLog(@"exception -> %@",[exception description]);
    } @finally {
    ...
    for (int i = 0; i < outCount; i ++) {
        ...
        if (isCan) {
            if (qhd_replaceMethod(aClass, selector, returnType)) {
                TLog(@"success hook method:%@ types:%s", NSStringFromSelector(selector), method_getDescription(tempMethod)->types);
            } else {
                TLog(@"fail method:%@ types:%s", NSStringFromSelector(selector), method_getDescription(tempMethod)->types);
            }
        } else {
            TLog(@"can not hook method:%@ types:%s", NSStringFromSelector(selector), method_getDescription(tempMethod)->types);
        }
        free(returnType);
    }
    free(methods);
